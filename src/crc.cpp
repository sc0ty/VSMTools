...
uint32_t TCRC::getCRC32()
{
    return crc32 ^ 0xffffffff;
}

uint32_t TCRC::add(const void *buf, uint32_t len)
{
    const uint8_t *b = (const uint8_t *)buf;

    for(uint32_t i=0; i<len; i++)
    {
        crc32 = (crc32 >> 8) ^ crc32_tab[ (crc32 & 0xff) ^ b[i] ];
    }

    return crc32 ^ 0xffffffff;
}

uint32_t TCRC::add(uint8_t data)
{
    crc32 = (crc32 >> 8) ^ crc32_tab[ (crc32 & 0xff) ^ data ];
    return crc32 ^ 0xffffffff;
}
